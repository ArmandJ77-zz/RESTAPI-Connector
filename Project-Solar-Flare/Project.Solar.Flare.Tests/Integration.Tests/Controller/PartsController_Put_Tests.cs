...
namespace Project.Solar.Flare.Tests.Integration.Tests.Controller
{
  ...
  public class PartsController_Put_Tests : BaseTest
  {
    ...
    public async Task Parts_BodyPost_Id()
    {
      ...
      var response = await Context.Client.PutAsync("api/PutParts/FromBody", stringContent);
    ...
    public async Task Parts_FormPost_Id()
    {
      using (var multipartFormDataContent = new MultipartFormDataContent())
      {
        ...
        var response = await Context.Client.PutAsync("api/PutParts/FromForm", multipartFormDataContent);
        response.EnsureSuccessStatusCode();
        var result = await response.Content.ReadAsStringAsync();
